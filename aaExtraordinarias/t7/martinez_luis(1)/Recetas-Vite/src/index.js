...
async function getRecetas() {
    try {
        ...
        console.log(`respuesta de la peticion ${resp.status}, ${resp.statusText}`)
        if (!resp.ok) {
            console.error(`error en la peticon para traer las tarjetas ${resp.status}, ${resp.statusText}`);
        } else {
            return resp.json();
        }
    } catch (error) {
...
function showcards(recetas) {
    const contenedor = document.getElementById("recipesContainer")

    // recetas.forEach((element) => {
    //     contenedor.appendChild(crearTarjeta(element));
    // });
    // 
...
function crearTarjeta() {
    let recipe = {//prueba para ver la tarjeta y comprobar la eliminacion de las mismas
    ...
    deleteBtn.addEventListener("click", (e) => {
        card.remove();//quitamos la tarjeta del contenedor

        //borramos el nombre de la receta que habia sido guardado al hacer el post
        localStorage.removeItem(recipe.name, recipe.category);

        const xhr = new XMLHttpRequest();
        xhr.open(`${SERVER}/recipes/${recipe.id}/true`);
        xhr.send();
