export function addItems(notas, item) {//parte las notas si estan pasadas por comas o una si solo se a pasado una
  return item.includes(",")
    ? notas.concat(item.split(",").map((nota) => nota.trim()))
    : notas.concat(item.trim());
}

export function clearItems(notas) {// verificamos si las notas entran del 0 al 10 y si es un numero 
  return notas
...
export function primerSuspenso(notas) {//si la nota es menor a 5 ,nos la devuelve
  return notas.find((nota) => nota < 5);
}
export function aprobados(notas) {//nos devuelve todas la nostas que este aprobadas
  return notas.filter((nota) => nota >= 5);
}

export function notaMedia(notas) {
